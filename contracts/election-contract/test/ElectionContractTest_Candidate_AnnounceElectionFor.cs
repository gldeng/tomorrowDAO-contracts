...
public class ElectionContractTestCandidateAnnounceElectionFor : ElectionContractTestBase
{
    [Fact]
    public async Task AnnounceElectionForTest()
    {
        var daoId = await InitializeContractAndCreateDao();

        var result = await AnnounceElection(daoId, UserAddress);
        result.ShouldNotBeNull();

        var candidateAdded = GetLogEvent<CandidateAdded>(result.TransactionResult);
        candidateAdded.ShouldNotBeNull();
        candidateAdded.Candidate.ShouldBe(UserAddress);
        candidateAdded.CandidateAdmin.ShouldBe(((MethodStubFactory)ElectionContractStub.__factory).Sender);
        ...
        candidates.Value.FirstOrDefault().ShouldBe(UserAddress);
